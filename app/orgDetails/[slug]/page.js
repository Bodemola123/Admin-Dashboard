...
const Page = ({ orgData }) => {
  ...
  const createdDate = orgData?.created_at?.split('T')[0];
  const updatedDate = orgData?.updated_at?.split('T')[0];

  // Format updated time with AM/PM
  let updatedTime = '';
  if (orgData?.updated_at) {
    const updatedDateObj = new Date(orgData.updated_at);
      ...
      <div className="px-8 py-4 flex flex-row justify-between border-[1.5px] border-[#EBF2FF] bg-[#ffffff] rounded-[17px]">
        <div className="flex flex-col gap-6 items-start">
          <p className="font-medium text-xl text-[#2d2d2d]">
            {orgData?.organization_name || 'No name inputted'}
          </p>
          <p className="font-medium text-base text-[#6D6D6D]">
