...
function ProductDetailsLayout() {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://2zztcz7h0a.execute-api.ap-southeast-2.amazonaws.com/default/voyex_tools_api?tool_id=${ProductId}`;
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        if (data.data.tool_id) {
          ...
          console.log("Fetched ProductData:", data.data); // 🔍 LOG HERE
        } else {
      ...
      <main className="w-full h-screen overflow-y-scroll overflow-x-hidden scrollbar-hide p-6 bg-[#F4F4F4]">
        {loading ? (
          <div className="flex flex-col gap-2 items-center justify-center min-h-full bg-white rounded-xl shadow-sm">
            ...
            <p className="text-gray-700 font-medium">Loading Product data...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col gap-2 items-center justify-center min-h-full bg-white rounded-xl shadow-sm">
